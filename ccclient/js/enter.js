...
{
	var change=true;
	if(n>=0&&jsclient.entercode.length<6)       jsclient.entercode.push(n);
    else if(n==-1&&jsclient.entercode.length>0) jsclient.entercode.length=jsclient.entercode.length-1;
	else if(n==-2&&jsclient.entercode.length>0) jsclient.entercode.length=0;
	else change=false;
	if(change) 
	{
