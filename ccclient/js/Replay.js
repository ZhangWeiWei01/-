...
//自动出牌
	...
	function SetPlayerVisible(node, off)
	{
		...
		/*///zys 解决，重复打开回放，显示upCard
		var upCard = head.getChildByName("up")
		...
		if (pl)
		{
			...
			setOffline(node, off);
			//InitPlayerHandUI(node, off); 删除再次播放回放时候的左面多余的几张牌，此处不应该调用这个函数
	...
	function HandleMJChi(node, msg, off)
	{
		...
		if (tData.curPlayer == selfIndex) {
			...
			//删掉俩张stand&&
			if (off == 3 && !pl.mjhand)      RemoveBackNode(node, "standPri", 2);
			else if (off != 0 && !pl.mjhand) RemoveFrontNode(node, "standPri", 2);
...
// mylog("== HandleMJGang ==" + off);
	...
	function RestoreCardLayout(node, off, endonepl) {
		...
		console.log("RestoreCardLayout , off == " , off , ", mjhandNum == " , mjhandNum );
	...
	function HandleMJPut(node, msg, off, outNum)
	{
		...
		if (uids[selfIndex] == msg.uid) {
			...
			////zys---add
			// 设置麻将上花色，参数1：node,参数2，具体花色，参数3：off，放心，也是设置花色角度
			///修改buff:牌的花色不对,通过修过第三个参数，off，算出一个正确的角度
			setCardPic(outAction, msg.card, 4);
...
//东南西北中发白
	...
	/*function setCardPic(node, cd, off)
	{
		...
		console.log("开始设置麻将图片------------------------- cd = " ,cd ,"  , off == " , off );
		if (cd < 30)
		{
		...
		node.tag = cd;
		////此处，zhangyisogn,修过，为什么要用回调设置 纹理？
		var callback = function ()
		{
			if(cd<100) {
				console.log(" 设置图片纹理------------- : " ,imgName);
			...
			if (btn.y >= standUI.y + 10)
			{
				if(tData.playType==PlayType.ylMethod)
				{
					...
					if(pl.isTing)
					{
						if(autoOutOn)
						{
							jsclient.replayui.jsBind.eat._node.stopAllActions();
							autoOutOn=false;
						}
					}
				}
		...
		var bind = {
			head: {
				...
				, coin: {
					_visible: true,
					_run: function () {
						console.log("-----wei ------wei --------pl = "+JSON.stringify(pl));
						console.log("-----wei ------wei --------tData = "+JSON.stringify(tData));
	...
	function InitPlayerHandUI(node, off) {
		...
		console.log("***********InitPlayerHandUI*********** 初始化------ off === " , off);
		var sData = jsclient.data.sData;
		var tData = sData.tData;
		var pl = getUIPlayer(off);
		if(!pl)return;
		...
		//添加手牌
		if (off == 0) {
			console.log("************** 自己的手牌数据 ===============  " ,pl.mjhand.length);
			for (var i = 0; i < pl.mjhand.length; i++) {
				if (off == 0) {
					AddNewCard(node, "stand", "mjhand", pl.mjhand[i], off);
				}
		...
		jsBind: {
			_event: {
				...
				, cardnumAtlas: {
					_text: function ()
					{
						...
						waitPut: function ()
						{/*
							console.log("---------------------waitput   event!");
							var sData = jsclient.data.sData;
							var tData = sData.tData;

							if (tData)
							{
								console.log("join...."+ PlayType.hsMethod);
								...
								if(jsclient.data.sData.tData.playType == PlayType.sxMethod){
									...
									if(tData.withWind){
										...
										console.log(""+tData.withZhong?cardNum+4:cardNum);
										this.setString(tData.withZhong?cardNum+4:cardNum);
									}
				...
				clt: {
					...
					/*play:{
						...
						canHu_hongzhong:{
							...
							_visible:function(){
								if (jsclient.data.sData.tData.playType==PlayType.sxMethod) return jsclient.data.sData.tData.withZhong;
								else return false;
							}
						},
						_visible:function(){
			...
			pao:{
				...
				_event:{
					downPao:function () {
						this.visible = true;
						jsclient.replayui.jsBind.wxinvite.visible = false;
						jsclient.replayui.jsBind.delroom.visible = false;
					}
					,reinitSceneData:function (eD) {
