...
function homeRunText(node,l)
{
	
	var  length = l*40+ node.getParent().getCustomSize().width;
	node.x =node.x+node.getParent().getCustomSize().width*1.5;
	...
	jsBind:{
		...
		createRoom:{
			_run:function()
			{
				...
				{
					doLayout(this,[0.43,0.43],[0.5,0.5],[0,-0.3] );
				}
			},
		...
		coinRoom:{
			_run:function()
			{
				...
				{
					doLayout(this,[0.43,0.43],[1,0.5],[-0.9,-0.3] );
				}
			},
